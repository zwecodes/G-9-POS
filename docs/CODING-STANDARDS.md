...
**Changelog since 1.3:** Conformance pass. §5.8 serializes `API-SPEC.md` §1.4 (`{ data, meta }` / `{ error, meta }`) — no `success` flag, `error` is an object. §4.3 standalone `PRODUCT_*` enqueue uses `referenceId: null` (`SYNC-PROTOCOL.md` §2.5). §1 rule 1 names the closed REST exceptions (setup, users, revoke, catalog import, auth, device rename). Catalog import and staff expenses match `API-SPEC.md` v1.6.
...
1. **Shop-data writes go through `POST /v1/sync/events` only.** No direct REST write endpoints for products, sales, void, expenses, suppliers, or orders. Closed exceptions, none of which the POS uses for day-to-day CRUD: auth, device rename, `POST /v1/setup`, `POST /v1/users`, `POST /v1/devices/{id}/revoke`, and `POST /v1/catalog/import` (which applies `PRODUCT_CREATED` / `CATEGORY_CREATED` / `INVENTORY_ADJUSTED` through the same event processor).
...
class ProductRepository {
  ...
  Future<void> saveProduct(ProductCompanion product) async {
    ...
    await _syncQueue.enqueue(
      ...
      referenceId: null, // standalone — never product.id (SYNC-PROTOCOL.md §2.5)
    );
  }
}
...
- Standalone events (`PRODUCT_*`, `CATEGORY_*`, `EXPENSE_*`, `DEVICE_ACTIVATED`, `INVENTORY_ADJUSTED` / `DAMAGED` / `RETURNED`) enqueue with `referenceId: null`. Grouped events (`SALE_CREATED` + `INVENTORY_SOLD`, void, supplier receipt) share the causal parent's id. Never copy the row's own `id` into `referenceId`.
...
**Rules:**
- **Default** sync shape is `createdAt`, `updatedAt`, `deletedAt`, `deviceId`. Tables that omit any of these are listed in `DATA-MODEL.md` §1.2 — do not add the omitted columns to those tables. In particular: `inventory_events` has no `updatedAt` / `deletedAt` (SQLite-only `rejectedAt` is not a soft-delete); `sale_items` and `supplier_order_items` omit `updatedAt`, `deletedAt`, and `deviceId`; `sync_queue` and `users` are not device-originated sync events and do not use that four-column shape.
- Filter `deletedAt.isNull()` on every read of a table that **has** `deletedAt`. Never expose soft-deleted records to the UI. `inventory_events` has no `deletedAt` — do not invent that predicate for it; stock reads use `rejectedAt.isNull()` instead (below).
- Never use auto-increment integer IDs — always UUID strings. Device-originated rows: generate on the device. Server-originated exceptions (`users`, catalog-import products/categories/inventory events) are listed in `DATA-MODEL.md` §1.2
...
All shop-data writes arrive through `POST /v1/sync/events`. Catalog import (`POST /v1/catalog/import`) calls this same processor internally — do not add a parallel product writer.
...
Matches `API-SPEC.md` §1.4. Success and error are distinguished by which key is present. There is no `success` boolean.
...
type APIError struct {
  ...
  // Reason-specific extras (retry_after, blocking_product_count, details)
  // are set on this object, not on a second envelope.
}
...
Never `c.JSON()` a flat `{success, data, error}` object. Never serialize `error` as a string.
...
**Correctness**
- [ ] Shop-data writes go through the sync queue — no direct REST product/sale/expense/order writes. Admin exceptions are only those listed in `API-SPEC.md` §1.1
