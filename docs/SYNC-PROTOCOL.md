...
**Changelog since 1.6:** Records the close of `DATA-MODEL.md` §9.1: `sales.server_received_at` is mirrored into SQLite and returned on `GET /v1/sync/pull` as server-originated sale metadata (including sales this device created). The server remains the sole enforcer of the void window. §2.5's C2 conflict note is removed — `CODING-STANDARDS.md` §4.3 now enqueues standalone events with `referenceId: null`. G1 / §11.2 behaviour is unchanged.
...
**Conformance (closed in `CODING-STANDARDS.md` v1.4).** The `ProductRepository.saveProduct` example now enqueues standalone `PRODUCT_*` events with `referenceId: null`. This section's rule did not move.
...
Response: all server-side changes since `last_sync_at` that this device hasn't originated, **plus** server-authored fields on rows this device did originate when those fields did not exist at write time.

In particular, each `sales` row includes `server_received_at` once the server has stamped the `SALE_CREATED` group (`DATA-MODEL.md` §3.6). That stamp is server-originated metadata: pull returns it to the originating device so SQLite can mirror it. The meaning of the field is unchanged — it is still the server clock at commit, still the void-window anchor, and it is still **not** sent up in the `SALE_CREATED` payload.

Pull also includes `users` rows needed for on-device PIN unlock (`id`, `name`, `pin`, `role`, timestamps, `deleted_at`). It never includes `password_hash`.
...
| Honest optimistic void | Optimistic local void remains. `VOID_WINDOW_CLOSED` still reverts per §4.4 / G1. `CANCELLED` is derived from `sales.status = voided` and is absent after revert restores `completed`. Deferred owner notice lives on the sync status screen — never a modal, never in the sale path. See §11.2 (closed in 1.6) |
| Mirrored `sales.server_received_at` | SQLite holds a copy for void-action UX after sync. Server remains the sole enforcer. Pull returns the stamp as server-originated metadata (`DATA-MODEL.md` §3.6, closed 1.7) |
...
**Resolution: keep optimistic local void; tell the truth in the UI; put the deferred notice on the existing sync status screen.** `VOID_WINDOW_CLOSED` continues to use the already-resolved §4.4 / G1 rollback. No pending badge. Presentation lives in `UI-GUIDELINES.md` (v1.1) §5.5, §5.8 and §6. Local void-action visibility may now use the mirrored `sales.server_received_at` (`DATA-MODEL.md` §3.6, closed 1.7); that does not reopen this decision.
...
1. **Optimistic local void remains.** The owner can still cancel a sale with no internet. After the stamp is mirrored, the device may hide the void action for UX (`DATA-MODEL.md` §3.6). That is not local authority: the server still refuses `VOID_WINDOW_CLOSED`, and a still-NULL stamp is still submitted optimistically.
...
4. **Mirroring `sales.server_received_at` is now decided** (`DATA-MODEL.md` §3.6 / §8, closed 1.7). It was not required to close *this* decision in 1.6, and closing it later does not replace §4.4. The server remains the sole enforcer.

**What this does not change.** G1, `rejected_at`, the `SALE_CREATED` / `SALE_VOIDED` payloads, the §4.4 revert table, or honest confirmation copy.